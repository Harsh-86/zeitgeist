...
THRESHOLD_RATIONALE = (
    "Floors are deliberately 0.95, not the plan's \"first-run rate rounded down to "
    "0.05\" rule (which would pin 1.00): generation runs at default temperature, so "
    "a 1.00 floor would fail CI on a single stochastic flake. 0.95 allows exactly one "
    "flaky question per suite while still catching any real regression — the bug this "
    "harness exists for cratered retrieval to 11.54%. Raising a floor is a deliberate "
    "commit."
)
